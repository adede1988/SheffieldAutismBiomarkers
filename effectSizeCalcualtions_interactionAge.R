...
for(ii in 2:length(ageGroups)){
  ...
  for(tt in 1:length(varNames)){
    ...
    curLM = lm(dv ~ sex + age * diag  +IQ , data = modDat)
    ...
    curLM = lm(dv2 ~ sex + age * diag  + IQ, data = modDat )
    aovTab = Anova(curLM, type = 3)
...
for(vari in 1:5){
  for(agei in 1:3){
    ...
    png(paste(figPath, 'interactionAGE_effect_', predNames[vari], '_', as.character(agei), '.png',sep=''),         # File name
...
allEffects = data.frame('agreement' = c(resultsDif$age, resultsDif$sex, resultsDif$IQ, resultsDif$sexDiag),
                        ...
                        'predictor' = c(rep('age', n), rep('sex', n), rep('IQ', n), rep('age X diagnosis', n)))
  ...
  #how well does stability predict effect size when age is the predictor? 
  cor.test(filter(allEffects, predictor == 'age X diagnosis')[,1],filter(allEffects, predictor == 'age X diagnosis')[,2])
...
png(paste(figPath, 'interactionAGE_effect_agree_relationship.png', sep = ''),         # File name
...
for(vari in 1:5){
  # for(agei in 1:3){
    ...
    png(paste(figPath, 'interactionAGE_stability_', predNames[vari], '_', '.png',sep=''),         # File name
        width=1024, height=768)
    print(outPlot)
    dev.off()       
  # }  
...
#     ADGroup = sample(biomark2[groupID2==2], round(baseRate*1000), replace = T )
#     CONGroup = sample(biomark2[groupID2==1], round((1-baseRate)*1000), replace = T)
